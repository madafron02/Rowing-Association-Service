...
public class PositionHandler implements FilteringHandler {
    ...
    public boolean handle(MatchFilter matchFilter) {
        if (matchFilter.getActivityApp().getPositions().containsKey(matchFilter.getUserPreferences().getPosition())
                && matchFilter.getActivityApp().getPositions().get(matchFilter.getUserPreferences().getPosition()) > 0) {
            if (next != null) {
